...
def setup_logging(
	name, level='info', log_file='gfx.log', root=False, console_log=True):
	...
	implies that it inherits its configuration to all loggers hierarchy, use this
	if you want to see logs from all modules, even external libraries. If set to
	False, the logger configured is the module's logger
	:param console_log: if True, the console handler will be added to the logger,
	which means that the log will also be shown in the screen, if False the
	messages will only be logged to files
	:return: returns the instance of the logger already configured
	"""
	...
	if root:
		...
		logger = logging.getLogger(name)
		# include log messages from common modules
		logging.getLogger('common').setLevel(level)
		logging.getLogger('common').addHandler(file_handler)
		logging.getLogger('common').addHandler(error_file_handler)
		logging.getLogger('gfx_qa_tools').setLevel(level)
		logging.getLogger('gfx_qa_tools').addHandler(file_handler)
		logging.getLogger('gfx_qa_tools').addHandler(error_file_handler)
		# add the console handler only if enabled
		if console_log:
			logging.getLogger('common').addHandler(console_handler)
			logging.getLogger('gfx_qa_tools').addHandler(console_handler)
	...
	# previous call) then just use that/those one(s) and don't add more handlers.
