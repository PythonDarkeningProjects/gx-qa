...
from gfx_qa_tools.config import defaults
...
def get(section, option, raw=False, variables=None):
	...
	Python3 uses a configparser that is different than the Python2 ConfigParser,
	it has some additional functionality. One of the biggest differences is that
	with configparser you can interpolate variables from one section into another
	section of the config file. This functionality is not included in the
	ConfigPrser. Since it is very useful, this function wrapps the original get
	function from ConfigParser so it includes the missing interpolation.
	...
	for match in matches:
		# interpolate matches found with ${something} with the referenced option
