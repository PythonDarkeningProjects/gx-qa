...
# https://lists.freedesktop.org/archives/xorg/2016-August/058168.html
# wget https://www.x.org/releases/individual/lib/libXfont2-2.0.1.tar.bz2 (./configure; make -j`nproc`; sudo make install)
# ==================================================================================================================================
# How to solve : Requested 'xproto >= 7.0.31' but version of Xproto is 7.0.29 (xserver driver)
# https://cgit.freedesktop.org/xorg/proto/x11proto/
# git clone https://anongit.freedesktop.org/git/xorg/proto/x11proto.git (./autogen.sh; ./configure; make -j`nproc`; sudo make install)
# or if the above step does not works, try this : https://packages.debian.org/sid/all/x11proto-core-dev/download
# wget http://ftp.us.debian.org/debian/pool/main/x/x11proto-core/x11proto-core-dev_7.0.31-1_all.deb
# ==================================================================================================================================
#
# (patches)
# main url : https://patchwork.freedesktop.org/
#
# (useful information)
# all drivers for xorg could be found here : https://cgit.freedesktop.org/xorg/driver/
# http://docs.astropy.org/en/stable/table/construct_table.html
# in order to run this script please make sure to do the following step
...
# cmd : sudo setfacl -Rdm g:groupnamehere:rwx /base/path/members/
#
# apt-get install libxcb-icccm4-dev (de aqui se pueden buscar los demas paquetes)
# https://www.x.org/releases/individual/xcb/
# No package 'xcb-renderutil' found (done)
# No package 'xcb-util' found (done)
# No package 'xcb-image' found (done)
# https://cgit.freedesktop.org/xcb/libxcb/
# http://www.linuxfromscratch.org/blfs/view/svn/x/xcb-proto.html
#
#
# Requested 'libinput >= 1.4.901' but version of Libinput is 1.4.3
# download the following packages from :https://polyakov.matem.unam.mx/ubuntu/pool/main/libi/libinput
# libinput10_1.6.3-1ubuntu1_amd64.deb
# libinput-bin_1.6.3-1ubuntu1_amd64.deb
# libinput-dev_1.6.3-1ubuntu1_amd64.deb
# Add libfontconfig1-dev version 2.11.94-0ubuntu1.1 library needed for compiling latest cairo versions
...
class Builder(object):
	...
	def __init__(self, **kwargs):
		...
		self.user = str(self.data['dut_config']['dut_user'])
		self.packageName = str(self.data['package_config']['package_name'])
		self.packageName = self.b.get_output('echo ' + self.packageName + "| sed -e s'| ||'g -e s'|_||'g -e s'|-||'g ") # Fixing the packageName
		self.latest_commits_value = self.data['latest_commits']['value']
		self.specific_commits_value = self.data['specific_commit']['value']

		# self.ip = self.b.get_output("/sbin/ifconfig | grep -Eo 'inet (addr:)?([0-9]*\.){3}[0-9]*' | grep -Eo '([0-9]*\.){3}[0-9]*' | grep -v '127.0.0.1'") # this returns two ip for bifrost.intel.com
		# === Getting the ip ===
		...
		self.clonexorgproto = 'https://anongit.freedesktop.org/git/xorg/proto/xorgproto.git'

		# === Build options ===
		self.cores = int(self.b.get_output('nproc'))
		self.cores = self.cores + 2
		self.debianRootPath = self.mainPath + '/tmp.drivers'
		self.masterLocation = '/opt/X11R7' # ====== change this variable if you need to install the graphic stack in other place ======

		#self.outputPath = self.mainPath + '/tmp.drivers' + self.masterLocation
		...
		# === Build parameters ===
		# (configure.ac) contains all the parameters availables for each driver
		self.eglPlatforms = 'x11,drm,wayland'

		#self.drmBuildParamters = './autogen.sh --prefix=' + self.prefix + ' --libdir=' + self.libdir
		self.drmBuildParamters = './autogen.sh --prefix=' + self.prefix + ' --libdir=' + self.libdir2


		#self.mesaBuildParameters_a = './autogen.sh --prefix=' + self.prefix + ' --libdir=' + self.libdir + ' --build=x86_64-linux-gnu --host=x86_64-linux-gnu --with-dri-driverdir=' + self.libdir + '/dri' # for 64bit
		#self.mesaBuildParameters_a = './autogen.sh --prefix=' + self.prefix + ' --libdir=' + self.libdir2 + ' --build=x86_64-linux-gnu --host=x86_64-linux-gnu --with-dri-driverdir=' + self.libdir + '/dri' # for 64bit
		...
		#mesaBuildParameters_a = './autogen.sh --prefix=' + self.prefix + ' --libdir=' + self.libdir + ' --build=x86-linux-gnu --host=x86-linux-gnu --with-dri-driverdir=' + self.libdir + '/dri' # for 32 bits
		self.mesaBuildParameters_b = '--enable-gles1 --enable-gles2  --enable-shared-glapi --with-platforms=' + self.eglPlatforms +  ' --enable-texture-float --enable-gbm --enable-glx-tls --without-gallium-drivers --enable-debug'
		...
		#self.xorgXserverMacrosBuildParameters_b = 'echo ' + self.password + ' | sudo -S bash ./util/modular/build.sh -m /home/' + self.user + '/intel-graphics/mesa ' + self.prefix + ' -p pre-xserver -n'
		...
		#self.xorgXserverBuildParameters = './autogen.sh --prefix=' + self.prefix + ' --libdir=' + self.libdir + ' --enable-config-udev --disable-config-hal --disable-unit-tests'
		#self.xorgXserverBuildParameters = './autogen.sh --prefix=' + self.prefix + ' --libdir=' + self.libdir + ' --enable-config-udev --disable-config-hal --disable-unit-tests --enable-debug --enable-config-udev-kms --with-default-xkb-rules=RULES --with-font75dpidir=' + self.outputPath + '/' + self.masterLocation + '/share/fonts/X11/75dpi --with-font100dpidir=' +  self.outputPath + '/' + self.masterLocation + '/share/fonts/X11/100dpi'
		##########################################################################################################################################################################################################################
		#self.xorgXserverBuildParameters = './autogen.sh --prefix=' + self.prefix + ' --libdir=' + self.libdir + ' --enable-config-udev --disable-config-hal --disable-unit-tests --enable-debug --enable-config-udev-kms'# chido
		##########################################################################################################################################################################################################################

		# our parameters
		#self.xorgXserverBuildParameters = './autogen.sh --prefix=' + self.prefix + ' --libdir=' + self.libdir + ' --enable-config-udev --disable-config-hal --disable-unit-tests --enable-debug --enable-config-udev-kms --enable-glamor --enable-systemd-logind=no --with-xkb-output=' + self.masterLocation + '/share/X11/xkb'
		# martin parameters

		# the parameters with specific commits
		#self.xorgXserverBuildParameters = './autogen.sh --prefix=' + self.prefix + ' --libdir=' + self.libdir + ' --enable-ipv6 --enable-dri --enable-xvfb --enable-composite --enable-xcsecurity --enable-libunwind --enable-xorg --enable-xephyr --enable-glamor --enable-xwayland --enable-kdrive --enable-kdrive-evdev --enable-kdrive-kbd --enable-kdrive-mouse --enable-config-udev --enable-systemd-logind --enable-suid-wrapper --enable-dri3 --disable-install-setuid --enable-record --disable-xfbdev --disable-xfake --disable-static --libexecdir=' + self.libdir + '/xorg-server --sysconfdir=' + self.prefix + '/etc --sysconfdir=' + self.prefix + '/etc --localstatedir='+ self.prefix + '/var --with-xkb-path=' + self.prefix + '/share/X11/xkb --with-xkb-output=' + self.libdir + '/xkb --with-fontrootdir=/usr/share/fonts --with-sha1=libgcrypt --enable-debug'

		# the parameters with latest commit automated for config.yml
		self.xorgXserverBuildParameters = './autogen.sh --prefix=' + self.prefix + ' --libdir=' + self.libdir + ' --enable-ipv6 --enable-dri --enable-xvfb --enable-composite --enable-xcsecurity --enable-libunwind --enable-xorg --enable-xephyr --enable-glamor -enable-kdrive --enable-config-udev --enable-systemd-logind --enable-suid-wrapper --enable-dri3 --disable-install-setuid --enable-record --disable-static --libexecdir=' + self.libdir + '/xorg-server --sysconfdir=' + self.prefix + '/etc --sysconfdir=' + self.prefix + '/etc --localstatedir='+ self.prefix + '/var --with-xkb-path=' + self.prefix + '/share/X11/xkb --with-xkb-output=' + self.libdir + '/xkb --with-fontrootdir=/usr/share/fonts --with-sha1=libgcrypt  --enable-glamor --enable-debug'
	...
	def environmentVariables(self):
		...
		# The following variables are new and they comes from FI
		#os.environ['PKG_CONFIG_PATH'] = os.environ['INSTALL_ROOT'] + '/lib/pkgconfig:/usr/share/pkgconfig:/usr/lib/x86_64-linux-gnu/pkgconfig:/usr/lib/pkgconfig' # this routes applies for ubuntu 16.10
		#The following variables are needed in order to use chamelium, please see the documentation below
		#https://www.chromium.org/chromium-os/testing/chamelium
		#os.environ['XMLRPC_CFLAGS'] = self.b.get_output('xmlrpc-c-config client --cflags')
		#os.environ['XMLRPC_LIBS'] = self.b.get_output('xmlrpc-c-config client --libs')
	...
	def message(self,messageType,message):
		if messageType == 'err':
			...
			print cyan + '>>> (success) ' + end + message
	...
	def checkIP(self):
		if self.ip.startswith('19'):
			# sedline connection
			os.system('unset http_proxy ftp_proxy all_proxy ALL_PROXY socks_proxy https_proxy no_proxy')
		else:
			# intranet connection (setting proxies)
			os.environ['ALL_PROXY'] = 'socks://proxy-socks.fm.intel.com:1080'
			os.environ['all_proxy'] = 'socks://proxy-socks.fm.intel.com:1080'
			os.environ['http_proxy'] = 'http://proxy.fm.intel.com:911'
			os.environ['https_proxy'] = 'https://proxy.fm.intel.com:911'
			os.environ['ftp_proxy'] = 'ftp://proxy.fm.intel.com:911'
			os.environ['socks_proxy'] = 'socks://proxy-socks.fm.intel.com:1080'
			os.environ['no_proxy'] = 'localhost,.intel.com,127.0.0.0/8,192.168.0.0/16,10.0.0.0/8'

	def checkOutput(self,output,driver,finalMessage):
		if output != 0:
			self.message('err','unable to compile (' + driver + ')')
			sys.exit()
		elif output == 0 and finalMessage:
			self.message('ok','the driver (' + driver + ') was compiled successfully')

	def checkOutputB(self,output,messageDone,messageFail):
		if output != 0:
			self.message('err',messageFail)
			sys.exit()
		elif output == 0:
			self.message('ok',messageDone)

	def checkRoot(self):
		if os.geteuid() == 0:
			self.message('err', '(' + self.scriptName + ') needs to be run as normal user')
			sys.exit()
	...
	def build(self):
		...
		{'xf86-video-amdgpu':self.cloneamdgpu},{'xf86-video-ati':self.clonexf86videoati},{'xf86-video-radeonhd':self.clonexf86videoradeonhd},{'xf86-video-modesetting':self.clonexf86videomodesetting},
		{'pixman':self.clonepixman},{'xf86-video-chips':self.clonexf86videochips},{'libxtrans':self.clonelibxtrans},{'libX11':self.clonelibX11},{'libXext':self.clonelibXext},{'dri2proto':self.clonedri2proto},
		{'libpciaccess':self.clonelibpciaccess},{'libva-utils':self.clonelibvautils},{'libinput':self.clonewaylandlibinput},{'libXfont':self.clonelibXfont},{'xf86-input-wacom':self.clonexf86inputwacom},{'xrdb':self.clonexrdb},
		{'xorgproto': self.clonexorgproto}]
		...
		if not self.packageName:
			...
			packageName = self.packageName + '-' + self.month.lower() + '-ww' + self.weekNumber + '-' + self.weekDay.lower() + '-' + self.hour + '-code-' + self.randonNumber
			fichCTRL.write('Package: ' + packageName + '\n')
