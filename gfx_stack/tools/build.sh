...
failed() {
	...
	((${build_fail_amounts}++))
	echo "***** $1 failed on $2/$3, total_fails are ${build_fail_amounts}"
    else
	exit 1
    fi
}
...
# #   build app pclcomp
  ...
  #  build app xdpyinfo
  #  build app xdriinfo
  #  bt fetch origin mesa_7_0_branch:mesa_7_0_branch || return 1
  #  build app xedit
  #  build app xev
  #  build app xeyes
  #  build app xf86dga
...
usage() {
    ...
    echo "  -p pre-xserver/after-xserver : some component should be updated before xserver build, and some should after."
}
...
date
build_fail_amounts=0
if [ ! "$PHASE" == "after-xserver" ]; then
build util macros
build_proto
build_lib
build_xserver
...
#build proto dri2proto
if [ ! "$PHASE" == "pre-xserver" ]; then
build_app
build_font
build_driver
fi
if [ ${build_fail_amounts} -gt 0 ]; then
  echo "FAIL"
  exit 1
else
  echo "SUCCESS"
