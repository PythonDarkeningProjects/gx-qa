...
function CheckDmesgInfo () {
 ...
 _DEBUG_LOG_FILE=${mPATH}/dmesg_debug.log
  ...
  if [ ${_d_emerg} -ne 0 ]; then
    echo -ne "${cyan}>>> (info)${nc} dmesg ${cyan}EMERGENCY${nc} : (${bold}${_d_emerg}${nc}) msg \n"
  ...
  if [ ${_d_alert} -ne 0 ]; then
    echo -ne "${cyan}>>> (info)${nc} dmesg ${blue}ALERT${nc}     : (${bold}${_d_alert}${nc}) msg \n"
  ...
  if [ ${_d_crit} -ne 0 ]; then
    echo -ne "${cyan}>>> (info)${nc} dmesg ${red}CRIT1CAL${nc}   : (${bold}${_d_crit}${nc}) msg \n"
  ...
  if [ ${_d_err} -ne 0 ]; then
    echo -ne "${cyan}>>> (info)${nc} dmesg ${red}ERROR${nc}   : (${bold}${_d_err}${nc}) msg \n"
  ...
  if [ ${_d_warn} -ne 0 ]; then
    echo -ne "${cyan}>>> (info)${nc} dmesg ${yellow}WARNING${nc} : (${bold}${_d_warn}${nc}) msg \n"
  ...
  if [ ${_d_notice} -ne 0 ]; then
    echo -ne "${cyan}>>> (info)${nc} dmesg ${yellow}NOTICE${nc}  : (${bold}${_d_notice}${nc}) msg \n"
  ...
  if [ ${_d_info} -ne 0 ]; then
    echo -ne "${cyan}>>> (info)${nc} dmesg ${yellow}INFO${nc}    : (${bold}${_d_info}${nc}) msg \n"
  ...
  if [ ${_d_debug} -ne 0 ]; then
    echo -ne "${cyan}>>> (info)${nc} dmesg ${blue}DEBUG${nc}   : (${bold}${_d_debug}${nc}) msg \n"
